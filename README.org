...
The goal of the final project is to let students employ the concepts
and techniques overviewed during the course to answer some specific
questions about a selected dataset. We expect each student to provide
us a *project proposal* with (a) the name of the dataset (and a quick
description about its semantics) and (b) a couple of questions to be
answered.  Once the student get his/her proposal approved by both
professors, he/she will need to provide us a laboratory notebook (the
iceberg) and a short report (the tip of the iceberg) about the
findings. Both documents should be made available in a public =GIT=
repository and can be written in any literate programming environment,
such as R Markdown (Rstudio), Jupyter, and Org-Mode. Both documents
should be fully reproducible with the corresponding tools (i.e., the
knitr of R); datasets might be placed in the same repository where the
documents are located.

The deadlines:
- Project proposal -- the noon of the 12th of November 2018
- Final project -- 30th of November, 2018

Evaluation process:

1. Scientific question: formulation of an initial hypothesis,
   proposition of a justified methodology, clear conclusion on the
   hypothesis.
2. Methodology question: how much the philosophy of the *literate
   programming* is employed in the investigation and how much
   reproducible it is.
3. Technical skills: statistical skills, management of raw data and
   data cleanup procedures (possibly using tidyr to get tidy data),
   data transformation (dplyr), and data visualization (ggplot).
4. Comunication: how much detailed and rich is the laboratory notebook
   (the iceberk -- the bulk of the investigation), considering the
   flow of idea and interpretation record, and how much consistent,
   concise, and clear it is the final report (the tip of the
   iceberg). The checklist for good graphics must be respected in all
   communication activities.

Each of the four aspects are equaly important in the evaluation process.
...
An e-mail should be sent to both professors by the deadline to inform
of the availability of the file version in your public GIT repository.

** Final project 2017/2                                           :noexport:
